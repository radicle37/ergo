...
This package contains compile-only TypeScript checks for the public API surface of `ergo-state` and `ergo-react`.

It is separate from `packages/ergo-state` and `packages/ergo-react` on purpose. Tests inside those packages are good for runtime behavior and source-level typing, but they can miss problems in the built public declarations or in the package `exports` field. This package imports both packages by their public names, the same way app packages do, so it checks the public API that consumers actually see.
...
Add a small `.ts` file under `src/`, import from `ergo-state` or `ergo-react`, and let `tsc` be the test runner.
