...
export const createErgoStoreSelectorEntries = <
  State extends object,
  Selectors extends ErgoStoreSelectorRecord<State>,
  AutoselectorKeys extends ErgoStoreAutoselectorKeys<State>
>(
  store: Pick<ErgoStoreReadableApi<State>, 'getInitialState'>,
  autoselectors: AutoselectorKeys | undefined,
  selectors: Selectors | undefined
): ErgoStoreSelectorEntries<State> => {
  const publicSelectorEntries: Record<string, ErgoStoreSelectorDefinition<State, unknown>> = {};
  const internalSelectorEntries: Record<string, ErgoStoreSelectorDefinition<State, unknown>> = {};
  ...
  (autoselectors ?? []).forEach(property => {
    ...
    const selectorEntry = {
      select: (state: State) => state[property]
    };
  ...
  Reflect.ownKeys(selectorInputs).forEach(selectorName => {
    ...
    const selectorEntry = normalizeSelectorInput(
      (selectorInputs as Record<string, ErgoStoreSelectorInput<State, unknown>>)[selectorName]
    );
