 ...
 */
export const subscribeWithEqualityFn = <S, T>(
  onSelectedValueChange: (selectedValue: T) => void,
  store: Pick<ErgoStoreReadableApi<S>, 'getState' | 'subscribe'>,
  selectorFn: (storeState: S) => T,
  equalityFn: (valueA: T, valueB: T) => boolean = Object.is
): (() => void) => {
