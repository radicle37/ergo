...
These notes are for developers changing `ergo-state`. The public README should stay focused on how to use Ergo from an application store.
...
Most Ergo tests live inside `packages/ergo-state`, where they can test runtime behavior and source-level types together. Some TypeScript problems only show up after another package imports Ergo through its package export, so there is also a small compile-only package that exercises the public API surface from a package-boundary perspective:
...
The package `exports` field exposes the vanilla root entry (`.`) and the narrow `./adapter-internal` support entry consumed by `ergo-react`. React-capable consumers should import from `ergo-react`; vanilla consumers should import from `ergo-state`. Keep `./adapter-internal` limited to the smallest surface extension packages need.
...
React should remain a peer of `ergo-react`, not `ergo-state`, so vanilla consumers do not install or load React-only dependencies.
...
- `packages/ergo-state/src/createErgoStore.test.ts` for source-level behavior and compile-time assertions near the implementation.
